...
const RegisterCollection: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const errorMessage = err?.response?.data?.message || "Error al cargar datos iniciales";
        showErrorToast({
          title: "Error",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!installment) {
      showErrorToast({ message: "No se pudo cargar la información de la cuota." });
      setLoading(false);
      return;
    }
    ...
    try {
      ...
      const errorMessage = error?.response?.data?.message || error?.message || "Error de sistema al registrar el cobro.";
      showErrorToast({
        title: "Error",
        message: errorMessage,
      });
        ...
        <div className="flex-1 flex flex-col" style={{ marginLeft: "18rem" }}>
          ...
          <div className="flex flex-col items-center justify-center py-8 flex-1">
            <div className="w-full max-w-5xl bg-white rounded-lg shadow p-8 text-center">
              <div className="text-lg text-gray-600">Cargando información de la cuota...</div>
            </div>
          </div>
        </div>
      </div>
                  ...
                  <div className="mb-6 p-4 bg-gray-50 rounded-lg border">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                      <div>
                        ...
                        <span className="ml-2 text-gray-900">{installment.installmentNumber}</span>
                      </div>
                      <div>
                        <span className="font-medium text-gray-700">Monto:</span>
                        <span className="ml-2 text-gray-900">${installment.amount.toLocaleString()}</span>
                      </div>
                      <div>
                        <span className="font-medium text-gray-700">Vencimiento:</span>
                        <span className="ml-2 text-gray-900">{installment.dueDate}</span>
                      </div>
                      <div>
                ...
                <form onSubmit={handleSubmit} className="space-y-6" noValidate>
                  ...
                  <div>
                    ...
                    <div className="mt-1 text-right">
                      <small className="text-gray-500">{form.receiptNumber.length}/255 caracteres</small>
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <input
                      type="text"
                      value={installment ? `$${installment.amount.toLocaleString()}` : ""}
                      readOnly
                      className="w-full px-4 py-3 border border-gray-300 rounded-lg bg-gray-50 text-gray-700 cursor-not-allowed"
                    />
                    <small className="text-gray-500 mt-1 block">Este monto no puede ser modificado</small>
                  </div>
