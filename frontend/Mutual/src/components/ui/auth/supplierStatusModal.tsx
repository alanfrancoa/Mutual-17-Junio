...
const SupplierStatusModal: React.FC<SupplierStatusModalProps> = ({
  isOpen,
  onClose,
  onConfirm,
  supplierName,
  ...
  const title = isDeactivate ? "Dar de Baja Proveedor" : "Reactivar Proveedor";
        ...
        <div className="px-6 pb-6 text-gray-700">
          <div className="mb-4 break-words max-w-full">
            <p className="block text-sm font-medium text-left text-gray-700 mb-2">
              ¿Desea {isDeactivate ? "dar de baja" : "reactivar"} al proveedor: <strong>{supplierName}</strong>?
            </p>
          </div>
          <p className="block text-sm font-medium text-left text-gray-700 mb-2">
            {isDeactivate
              ? "Esta acción impedirá que el proveedor participe en nuevas operaciones y servicios."
              : "Esta acción permitirá que el proveedor vuelva a estar activo en el sistema y pueda brindar servicios."}
          </p>
          {modalError && (
