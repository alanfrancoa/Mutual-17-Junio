...
const ServiceStatusModal: React.FC<ServiceStatusModalProps> = ({
  ...
  const title = isDeactivate ? "Dar de Baja Servicio" : "Reactivar Servicio";
        ...
        <div className="px-6 pb-6 text-gray-700">
          <div className="mb-4 break-words max-w-full">
            <p className="block text-sm font-medium text-left text-gray-700 mb-2">
              ¿Desea {isDeactivate ? "dar de baja" : "reactivar"} el servicio: <strong>{serviceName}</strong>?
            </p>
          </div>
          <p className="block text-sm font-medium text-left text-gray-700 mb-2">
            {isDeactivate
              ? "Esta acción impedirá que el servicio esté disponible para nuevas operaciones y registros."
              : "Esta acción permitirá que el servicio vuelva a estar disponible en el sistema."}
          </p>
          {modalError && (
